pipeline {
    ...
    stages {
        ...
        stage('Docker build Image') {
            steps {
                dir("${params.typeApp}") {
                    sh "docker build --build-arg \"image=${params.image}\" --build-arg \"Maitener=${params.Maitener}\" --build-arg \"image=${params.image}\" -t ${params.newImage}:${params.newTag} -f Dockerfile ."
                }
            }
        }
        stage('Docker login') {
